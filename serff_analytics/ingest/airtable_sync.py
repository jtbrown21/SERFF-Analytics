...
class AirtableSync:
    ...
    def _fetch_records(self, since: datetime | None = None):
        """Stream records page by page from Airtable."""

        params: dict[str, str] = {}
        if since is not None:
            iso = since.isoformat()
            params["filter_by_formula"] = f"LAST_MODIFIED_TIME() > '{iso}'"
    ...
    def sync_data(self, since: datetime | None = None):
        ...
        try:
            ...
            record_count = sum(len(p) for p in pages)
            logger.info(f"Fetched {record_count} records from Airtable")
